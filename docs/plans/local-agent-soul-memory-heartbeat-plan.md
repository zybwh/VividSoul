...
7. 为降低模型漂移风险，不要求单轮聊天同时输出大而全的结构化 `JSON`。用户可见回复继续保持纯文本；结构化理解优先走 provider 原生 `tool/function calling`，只在兼容性需要时保留极小、扁平 `JSON` 的降级解析。
...
### 4.1 `LLM-first` 语义判断，`Markdown + tool call` 优先

- 自然语言理解优先交给专用 `LLM judge`
- 不用大量规则、正则或句式模板去硬判复杂语义
- 长期可读知识优先用 `Markdown`
- 结构化执行优先用 provider 原生 `tool/function calling`
- 可靠执行状态优先用 `JSON/JSONL`
- 不设计大而全、层级很深的统一结构
- 所有结构化 `judge / summarizer` 都必须是“单任务、扁平、少字段、可重试”的；若必须使用文本协议，也只能是极小兼容层
...
- `habits.md`
  ...
  - 不随模型切换而丢失
- `habit-candidates.json`
  - `inferred_habit_candidate` 的后台候选池
  - 不直接进入主 prompt，重复命中后再晋升到 `habits.md`
- `bond.md`
  - 该模型与该用户的专属关系摘要
...
- 老对话 compact
- `MemoryJudge` 输出的 `inferred_habit_candidate` 先写入 `habit-candidates.json`，重复命中后再晋升到 `habits.md`
- `habits.md` 汇总
- `bond.md` 汇总
- `facts.md` 归并与去重
- 索引刷新
...
- 单一职责
- 最多返回少量操作
- 优先通过单次 `tool_call` 提交参数，而不是在 `message.content` 里打印 `JSON`
...
- reminder 不落盘
- memory 写入可跳过
- 记录轻量诊断日志，便于后续修正 prompt / tool schema
...
- `MemoryJudge` 已进入第二轮质量收敛，包含本地 confidence gate、覆盖命中、后台写入，以及 `inferredHabitCandidate -> habit-candidates.json -> habits.md` 的候选晋升链路
- `ReminderIntentJudge`、`ReminderStore`、`ReminderScheduler` 已开始接入当前本地 backend
- 第一版 reminder 触发先复用现有聊天消息 / HUD / 对白气泡通道，不单独引入新的通知壳层
...
- 已有专用 `ReminderIntentJudge`
- 已有本地持久化 `ReminderStore`
- 已有本地 `ReminderScheduler`，通过当前活跃本地 backend 的轮询驱动到期扫描
- reminder 触发已能回流到统一 `ConversationMessageEnvelope`，复用聊天记录与对白气泡
- pending reminder 已开始回注到本地 prompt，作为“我让你提醒了什么”这类问题的第一层 grounding
- 仍需补足真实 provider 回归、取消/完成匹配质量、以及更完善的启动/焦点恢复验证
...
- 拆成多个单一职责 extractor
- schema 保持扁平、字段少
- 优先使用 provider 原生 `tool/function calling`
- parse 失败不阻断主回复
...
3. 用专用 reminder extractor、原生 `tool/function calling` 和本地调度器把 `heartbeat` 做成可靠提醒系统。
